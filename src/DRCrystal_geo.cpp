...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens)  {
  ...
  //the assembled tower is a Stave
    ...
    // tower envelope
  dd4hep::Box towertrap(hwidth+tol,hwidth+0.1+0.4+0.1+0.2+tol,hzmax+0.1+0.4+tol);
  ...
  for(xml_coll_t li(x_det,_U(layer)); li; ++li)  
  {
	...
	// find top and bottom lengths at this position and center
      ...
      dd4hep::Box l_box(hwidth,hwidth+0.1+0.4+0.1+0.2,l_hzthick+0.1+0.4);
...
//Right Silicone slice
int s_num_right = s_num_1;
double z_bott2_right=l_hzthick;
for(xml_coll_t si_z_1(x_layer,_U(end)); si_z_1; ++si_z_1)  
{
	std::cout<<" with slice "<<s_num_right<<std::endl;
	xml_comp_t x_module_1 = si_z_1;
	string     s_name_z_1  = _toString(s_num_right,"slice%d");
	double     s_hzthick_z_1 = x_module_1.thickness()/2.;
	std::cout<<" with half  thickness "<<s_hzthick_z_1<<std::endl;

	double z_mi2_right = z_bott2_right + s_hzthick_z_1;
	Position   s_pos(0.,0.,z_mi2_right);      // Position of the slice (?)
	std::cout<<" placed at "<<z_mi2_right<<std::endl;
	dd4hep::Box s_box(2.5/2,2.5/2,s_hzthick_z_1);
	...
	slice.setAttributes(description,s_vol,x_module_1.regionStr(),x_module_1.limitsStr(),x_module_1.visStr());
          ...
          // Slice placement.
	PlacedVolume slice_phv = l_vol.placeVolume(s_vol,s_pos);
	slice_phv.addPhysVolID("slice", s_num_right);
	slice.setPlacement(slice_phv);
          // Increment Z position of slice.

	z_bott2_right += 2.*s_hzthick_z_1;
...
int s_num_z_1 = s_num_right; //Whatever this maximum value of s_num is after the previous loop, increment that so there is no overlap
//double y_bottoms2_1=hwidth;  
//int s_num_z_1 = s_num_y_3;
...
int s_num_z_4_filter = s_num_z_4;
double y_bot_filter=hwidth;
for(xml_coll_t si_y_1(x_layer,_U(pad)); si_y_1; ++si_y_1)  
{
	std::cout<<" with slice "<<s_num_z_4_filter<<std::endl;
	xml_comp_t x_shape_1 = si_y_1;
	string     s_name_y_1  = _toString(s_num_z_4_filter,"slice%d");
	double     s_hzthick_y_filter = x_shape_1.thickness()/2.;
	std::cout<<" with half  thickness "<<s_hzthick_y_filter<<std::endl;

	double y_mid_filter = y_bot_filter + s_hzthick_y_filter;
	      

	Position   s_pos(0.,y_mid_filter,0.);      // Position of the slice (?)
	std::cout<<" placed at "<<y_mid_filter<<std::endl;
	//dd4hep::Tube s_tube(0.,1.25,s_hzthick_y_filter,0.*deg,360*deg);
	dd4hep::Box s_box(2.5/2,s_hzthick_y_filter,2.5/2);
	...
	slice.setAttributes(description,s_vol,x_shape_1.regionStr(),x_shape_1.limitsStr(),x_shape_1.visStr());
          ...
          // Slice placement.
  //Transform3D rot(dd4hep::RotationZYX(M_PI/2.,0.,0.),s_pos);
	PlacedVolume slice_phv = l_vol.placeVolume(s_vol,s_pos);
  //PlacedVolume slice_phv = l_vol.placeVolume(s_vol,rot);
	slice_phv.addPhysVolID("slice", s_num_z_4_filter);
	slice.setPlacement(slice_phv);
          // Increment Z position of slice.

	y_bot_filter += 2.*s_hzthick_y_filter;
...
//SiPM 1 + Silicone gap 1
int s_num_y_1 = s_num_z_4_filter; //Whatever this maximum value of s_num is after the previous loop, increment that so there is no overlap
//double y_bottoms2_1=hwidth;  
double y_bottoms2_1=y_bot_filter;
...
double y_bottoms2_2=y_bot_filter;
...
double y_bottoms2_3=y_bot_filter;
...
double y_bottoms2_4=y_bot_filter;
